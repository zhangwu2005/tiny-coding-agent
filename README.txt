...
3. 执行：
   ...
   命令默认需人工确认；--auto-approve 可自动批准非高风险命令。--verification-policy none|syntax|test|full 设置验证门槛，默认 test；证据不足时须由用户输入 accept 或显式使用 --accept-incomplete。--context-limit 可调整发送给模型的上下文字符预算。
...
控制器按当前代码版本记录语法检查、静态分析、定向测试和完整测试集；旧版本证据不会证明新代码，弱检查不能掩盖更强测试失败。控制器判断验证是否充分，证据不足时用户拥有最终验收权。命令有风险分级与人工批准，测试失败会触发带诊断证据的反思检查点，重复工具调用会熔断。
